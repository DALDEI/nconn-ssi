...
replace(substring(substring_index(`event_log`.`detail`, '|', 3), length(substring_index(`event_log`.`detail`, '|', 3 - 1)) + 1), '|', '') as 'Opp ID List',
...
and  date(`event_log`.`event_dt`) >= :start_date
and date(`event_log`.`event_dt`) <= :end_date