...
ifnull(all_elem, 0) as 'Total Members', ifnull(new_elem, 0) as 'New Members (added in the report month)', report_month, report_year
from `campaign`
...
left join
(select `campaign_dist_elem`.`campaign_id`, COUNT(`campaign_dist_elem`.`campaign_dist_elem_id`) as new_elem, month(`campaign_dist_elem`.`ctime`) as report_month, year(`campaign_dist_elem`.`ctime`) as report_year
from `campaign_dist_elem`
where month(`campaign_dist_elem`.`ctime`) = 1
and year(`campaign_dist_elem`.`ctime`) = 2018
and `campaign_dist_elem`.`active` = 1
