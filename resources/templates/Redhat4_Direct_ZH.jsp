...
根据我们的记录，您的红帽订阅将于今天到期。请务必在截止日期前续约您的订阅以便继续获得红帽软件，    更新，升级，技术支持，安全补丁和Red Hat's Open Source Assurance Program合法使用。请阅读以下条款以便充分理解我们续订产品的好处。如果您有任何问题，请通过电子邮件与我联系 <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><a href=mailto:<x:out select="$d//ssirep/email"/>><x:out select="$d//ssirep/email"/></a></x:when><x:otherwise><a href=mailto:<x:out select="$d//client/rep_email"/>><x:out select="$d//client/rep_email"/></a></x:otherwise></x:choose>

</c:when>
<c:when test="${tpl_vals[2] == 'minus30'}"> 
根据我们的记录，您的红帽订阅已过期。请务必在截止日期前续约您的订阅以便继续获得红帽软件，更新，升级，技术支持，安全补丁和Red Hat's Open Source Assurance Program合法使用。请阅读以下条款以便充分理解我们续订产品的好处。如果您有任何问题，请通过电子邮件与我联系 <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><a href=mailto:<x:out select="$d//ssirep/email"/>><x:out select="$d//ssirep/email"/></a></x:when><x:otherwise><a href=mailto:<x:out select="$d//client/rep_email"/>><x:out select="$d//client/rep_email"/></a></x:otherwise></x:choose>
</c:when>

<c:otherwise> 
根据我们的记录，您的红帽订阅将在 <c:out value="${stage}"/> 过期。请务必在截止日期前续约您的订阅以便继续获得红帽软件， 更新，升级，技术支持，安全补丁和Red Hat's Open Source Assurance Program合法使用。 请阅读以下条款以便充分理解我们续订产品的好处。如果您有任何问题，请通过电子邮件与我联系 <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><a href=mailto:<x:out select="$d//ssirep/email"/>><x:out select="$d//ssirep/email"/></a></x:when><x:otherwise><a href=mailto:<x:out select="$d//client/rep_email"/>><x:out select="$d//client/rep_email"/></a></x:otherwise></x:choose>
...
<tr><td><font face="Arial" size="2">为了您继续享受订阅的服务，请联系您的经销商，或者通过邮件<a href=mailto:renewal-apac-partners@redha.com>renewal-apac-partners@redha.com</a> 联系我们的合作伙伴。<br><br></font></td></tr>
...
<tr><td><font face="Arial" size="2">红帽订阅帮助您解决使用红帽产品过程中出现的并提供持续和稳定的业务价值。红帽提供的服务价格是由所运行Red Hat软件系统的数量为计算单位，所以在您与红帽签订协议的期间即当您持有任何有效红帽订阅期间，您必须保证对每一个安装或使用的红帽系统和相关软件拥有合法和有效的订阅。在合法和有效的订阅服务期内方可获得如下权利：<br><br></font></td></tr>
<tr><td><font face="Arial" size="2"> 
<ul>
<li>
...
<tr><td><font face="Arial" size="2">如果您需要任何额外的信息，或者对我们服务的其他信息，请通过电话与我们联系  <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><x:out select="$d//ssirep/phone"/></x:when><x:otherwise><x:out select="$d//client/rep_tel"/></x:otherwise></x:choose>。
