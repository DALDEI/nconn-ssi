...
Red Hat yazılımı, güncelleme, üst versiyona geçiş, teknik destek, güvenlik yamaları ve Red Hat’in Open Source Assurance Programından yararlanmaya devam edebilmek için lütfen üyeliğinizi hemen yenileyin.  Değişen ürün yapılandırmalarımız ile yenileme şartlarını tamamen anlamak için lütfen aşağıdaki şartları okuduğunuzdan emin olun. Red Hat satış temsilciniz olarak her hangi bir sorunuz olursa <a href=mailto:<x:out select="$d//client/rep_email"/>><x:out select="$d//client/rep_email"/></a> adresinden bize ulaşabilirsiniz  
...
<th  align="Center" valign="center"><font face="Arial" size="2" color="ffffff">Sipariş<br> Numarası<br> Satınalma</font></th>
...
<c:choose><c:when test="${tpl_vals[2] == '0'}"><a href ="customerdata-emea@redhat.com">customerdata-emea@redhat.com</a></c:when><c:otherwise><a href ="customerdata-emea@redhat.com">customerdata-emea@redhat.com</a></c:otherwise></c:choose> adresine bildiriniz.
<c:choose>
<c:when test="${tpl_vals[2] == 'minus30'}"></c:when>
<c:otherwise>  Red Hat Login ID ve kontrakt numaranızı eklediğinizden emin olun.</c:otherwise>
</c:choose>
</font><br><br></td></tr>

<tr><td><font face="Arial" size="2">

İlave bir bilgi veya diğer servislerimiz hakkında bilgi almak isterseniz aşağıdaki adresten bize ulaşabilirsiniz 
