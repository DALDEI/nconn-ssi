...
Hemos pensado que le gustaría saber que sus suscripciones a Red Hat que se describen en el programa han caducado.  </c:when>
...
<c:otherwise> 
Hemos pensado que le gustaría saber que sus suscripciones a Red Hat que se describen en el programa van a caducar. </c:otherwise>
</c:choose>

Por favor, asegúrese de renovar sus suscripciones <c:choose><c:when test="${tpl_vals[2] == '0' || tpl_vals[2] == 'minus30'}">inmediatamente</c:when><c:otherwise>antes de la fecha de caducidad</c:otherwise></c:choose> para seguir recibiendo los beneficios de sus suscripciones, incluyendo el acceso al software de Red Hat, actualizaciones, mejoras, <c:if test="${tpl_vals[0] == 'NALA'}">nuevas versiones, </c:if>soporte técnico, revisiones de seguridad y el Programa de Seguridad Open Source de Red Hat. Asegúrese de leer los términos y condiciones de más abajo para entender completamente los términos de su oferta de renovación cuando cambien nuestras configuraciones de producto.

</font></td></tr>

<tr><td><font face="Arial" size="2"><br>
Para poder seguir recibiendo los beneficios de su suscripción, contacte con su Revendedor o, alternativamente, contacte con nuestro equipo de renovaciones en <c:if test="${tpl_vals[0] == 'NALA'}"><a href=mailto:latampartnerhelpdesk@redhat.com>latampartnerhelpdesk@redhat.com</a>.</c:if>
<c:if test="${tpl_vals[0] == 'EMEA'}"><a href=mailto:renewals-emea@redhat.com>renewals-emea@redhat.com</a>.</c:if>

</font></td></tr>

<c:if test="${tpl_vals[0] == 'EMEA'}">
<tr><td><font face="Arial" size="2"><br>
También me gustaría comunicarle que hemos realizado varias mejoras para proporcionarle más flexibilidad y valor desde su suscripción a Red Hat. Esto le proporcionará una versión más personalizada y mejorada de su solución Red Hat actual. Esta transición incluye nuevas configuraciones de producto más flexibles, números de producto, precios, niveles de apoyo y productos superpuestos opcionales. Como resultado de las nuevas configuraciones, su precio de renovación puede variar de su precio de compra original.</font></td></tr>
</c:if>

<c:if test="${tpl_vals[0] == 'NALA'}">
<tr><td><font face="Arial" size="2"><br>
También me gustaría comunicarle que hemos realizado varias mejoras para proporcionarle más flexibilidad y valor desde su suscripción a Red Hat. Esto le proporcionará una versión más personalizada y mejorada de su solución Red Hat actual. Esta transición incluye nuevas configuraciones de producto más flexibles, números de producto, precios, niveles de apoyo y funcionalidades opcionales adicionales. Como resultado de las nuevas configuraciones, su precio de renovación puede variar de su precio de compra original.
</font></td></tr>
</c:if>


<tr><td><font face="Arial" size="2"><br> 
La siguiente tabla le muestra el mapa de su suscripción actual a nuestra nueva estructura de producto, que se le entregará en cuanto renueve su suscripción:</font></td></tr>
...
<tr><td><font face="Arial" size="2"><br>
Obtenga más información sobre nuestra nueva estructura de producto en  <a href ="http://www.redhat.com/rhel/renew/">http://www.redhat.com/rhel/renew/ </a>
<br><br></font></td></tr>
...
<tr><td><font face="Arial" size="2">Puede recibir más información sobre los beneficios y requisitos de renovación de su suscripción a Red Hat haciendo clic <a href="http://www.redhat.com/subscriptions/">aquí</a>. Si necesita información adicional o información sobre nuestros servicios, contacte con su revendedor.</font><br><br></td></tr>

<tr><td><font face="Arial" size="2">Si no tiene un proveedor preferido de productos Red Hat, contacte con nuestro equipo de renovaciones en <c:if test="${tpl_vals[0] == 'EMEA'}"> <a href=mailto:renewals-emea@redhat.com>renewals-emea@redhat.com</a></c:if> 
<c:if test="${tpl_vals[0] == 'NALA'}"> <a href=mailto:latampartnerhelpdesk@redhat.com>latampartnerhelpdesk@redhat.com</a></c:if> y estaremos encantados de ponerle en contacto con un revendedor adecuado de Red Hat.</font><br><br></td></tr>

<tr><td><font face="Arial" size="2">
Si cree que alguien más debe recibir notificaciones futuras sobre sus suscripciones Red Hat, envíe los datos a <c:if test="${tpl_vals[0] == 'EMEA'}"> <a href=mailto:renewals-emea@redhat.com>renewals-emea@redhat.com</a></c:if><c:if test="${tpl_vals[0] == 'NALA'}"> <a href=mailto:customerservice-latam@redhat.com>customerservice-latam@redhat.com</a></c:if>. Asegúrese de incluir su identificación de acceso de Red Hat y su número de contrato.</a>
...
<tr><td><font face="Arial" size="2">
<c:if test="${tpl_vals[0] == 'EMEA'}">
El Equipo de Renovaciones de Red Hat
</c:if>
<c:if test="${tpl_vals[0] == 'NALA'}">
