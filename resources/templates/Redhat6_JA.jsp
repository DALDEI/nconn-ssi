...
<c:choose>
  ...
  <%-- REGION = APAC + COUNTRY = JAPAN Logic --%>
    ...
    <%-- DISCLAIMER - only used in Japanese template --%>
    <c:choose>
      <c:when test="${ (channel == 'Channel Tier 1') || (channel == 'Channel Tier 2') }"> 
        <c:set var="disclaimer" value="当該型番および本数は、VMware/ Hyper-VのゲストOSとしてRHELを使用しているケースを想定しています。KVMにより仮想化環境を構築している場合は、サブスクリプションの必要本数が異なってまいりますので、レッドハット販売店までご確認くださいますようお願い申し上げます。"/>
      </c:when>
      <c:otherwise>
        <c:set var="disclaimer" value=""/>
      </c:otherwise>
...
<table cellspacing="0" cellpadding="0" width="660" border="0">
  <tr><td><font face="Arial" size="2">Red Hat製品をご利用いただき、ありがとうございます。</font></td></tr> 
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>

  <c:choose>
    <c:when test="${tpl_stage == 'minus30'}">
      <tr><td><font face="Arial" size="2">以下のお客様のRed Hatサブスクリプションの有効期間はすでに終了しております。

</font></td></tr>
    </c:when>
    <c:when test="${tpl_stage == '0'}"> 
      <tr><td><font face="Arial" size="2">以下のお客様のRed Hatサブスクリプションの有効期限は本日までです。</font></td></tr> 
    </c:when>
    <c:otherwise> 
      <tr><td><font face="Arial" size="2">以下のお客様のRed Hatサブスクリプションの有効期間はあと<c:out value="${tpl_stage}"/> 日
...
間です。
  ...
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>
  <tr><td><font face="Arial" size="2">お客様名 -  <x:out select="$d//customer/name"/></font></td></tr>
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>
</table>
...
<table width=660 cellspacing="0" cellpadding="5" indent=30  border="0" >
  <tr bgcolor="CC0000">
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">数量</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">製品名</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">有効期限日</font></th>
  </tr>
        ...
        <tr>
          ...
          <td align="center" valign="center"><font face="Arial" style="font-size:8pt">
            ...
            <c:if test="${fn:length(enddate) gt 0}">
              <%= nexstra.cds.servlet.TemplateTags.reformatDate( pageContext , "enddate", "yyyyMMdd" , "yyyy MMM dd") %>
            </c:if>
          </font></td>
        </tr>
      </c:when>
...
<table cellspacing="0" cellpadding="0" width="660" border="0">
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>
  <tr><td><font face="Arial" size="2">Red Hatサブスクリプションの更新及びお問い合わせの際は、下記までご連絡下さい。</font> 
  ...
  <tr><td><font face="Arial" size="2">
    <blockquote>
      <c:if test="${fn:length(s_company) gt 0}">
        <c:out value="${s_company_preamble}"/><c:out value="${s_company}"/><br>
      </c:if>
      ...
      <c:if test="${fn:length(renew_online_url) gt 0}">
        ...
        <a href=<c:out value="${renew_online_url}"/>><c:out value="${renew_online_url}"/></a><br>
      </c:if>
      <c:out value="${s_phone}"/><br>
      <c:out value="${main_s_phone}"/><br>
    </blockquote>
  </font> </td></tr> 
</table>
...
<table cellspacing="0" cellpadding="0" width="660" border="0">
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>
  <tr><td><font face="Arial" size="2">
Red Hatサブスクリプションによりお客様のビジネスを継続的にご支援するため、ぜひご更新下さい。
ご更新いただきますと、下記のサービスが継続して受けられます。
ご更新の手続きが遅れますと、そのご契約の有効期限の翌日までご更新の開始日を遡る場合がございますのでご注意下さい。
  </font></td></tr>
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>
  <tr><td><font face="Arial" size="2"> 
    <ul>
      <li><b>セキュリティ: </b>Red HatのSecurity Response Team では、問題の98%以上を報告された日の翌日中に修正してきた実績が

あります。</li>
      <li><b>受賞歴のあるサポート: </b>Red Hatテクニカルサポートは、グローバルに24時間稼働しています。また、Red Hatカスタマーポータル

経由でナレッジベース、ケース

スタディやリファレンスアーキテクチャを活用できます。</li>
      <li><b>無償のアップデートおよびアップグレード: </b>サブスクリプションをお持ちの場合、サポート期間内のRed Hatソフトウェア及びすべての
...
ビスの提供を受けられます。</li>
  ...
  <tr><td><font face="Arial" size="2">Red Hatサブスクリプションのメリットについて詳しくは以下をご覧ください。</font></td></tr>
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>
  <tr><td><font face="Arial" size="2"><a href ="http://red.ht/1EGEUMm" >http://red.ht/1EGEUMm</a></font></td></tr>
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>

  <tr><td><font face="Arial" size="2">なお、本メールと行き違いで既に更新していただいている場合は、なにとぞご容赦ください。
  ...
  <tr><td><font face="Arial" size="2">更新及びお問い合わせの際は、以下の情報をご用意ください。</font> </td></tr> 
  <tr><td><font face="Arial" size="2">お客様の名前 - <x:out select="$d//customer/name"/></font></td></tr> 
...
<table width=660 cellspacing="0" cellpadding="5" indent=30 border="0">
  <tr bgcolor="CC0000">
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">ご契約番号</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">現在の注文番号</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">現在の数量</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">現在の製品型番</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">現在の製品</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">今回ご注文の数量</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">更新製品型番</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">更新製品名</font></th>
    <th align="center" valign="center"><font face="Arial" size="2" color="ffffff">有効期限日</font></th>
  </tr>
...
<table cellspacing="0" cellpadding="0" width="660" border="0">
  <tr><td><font face="Arial" size="2">&nbsp;</font></td></tr>
  <tr><td><font face="Arial" size="2"><c:out value="${disclaimer}"/></font></td></tr>
</table>
...
© 2015 Red Hat, Inc. All rights reserved. Red Hat、Red Hat Enterprise Linux、Shadowmanロゴ、およびJBossは、米国およびその

他の国におけるRed Hat, 

Inc.の登録商標です。Linux®は、米国およびその他の国におけるLinus Torvalds氏の登録商標です。その他の商標は該当する各社が所有して

います。
レッドハット製品のライフサイクルに関する情報はこちらをご覧ください。　https://access.redhat.com/support/policy/update_policies.html
  </font> </td></tr> 
</table>
