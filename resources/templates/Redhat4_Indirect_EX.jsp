...
Para poder seguir recibiendo los beneficios de su suscripción, contacte a nuestro equipo de renovaciones en <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><a href=mailto:<x:out select="$d//ssirep/email"/>><x:out select="$d//ssirep/email"/></a></x:when><x:otherwise><a href=mailto:<x:out select="$d//client/rep_email"/>><x:out select="$d//client/rep_email"/></a></x:otherwise></x:choose>.</font></td></tr>

</c:if>

<c:if test="${tpl_vals[0] == 'EMEA'}"><tr><td><font face="Arial" size="2"><br>Para poder seguir recibiendo los beneficios de su suscripción, contacte con su Revendedor o, alternativamente, contacte con nuestro equipo de renovaciones en <a href=mailto:renewals-emea@redhat.com>renewals-emea@redhat.com</a>.</font></td></tr></c:if>
