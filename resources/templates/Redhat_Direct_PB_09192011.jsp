...
<tr><td><font face="Arial" size="2">
De acordo com nossas informações, sua subscrição Red Hat vai EXPIRAR HOJE. Por favor certifique-se em renova-las imediatamente para que você continue recebendo os benefícios de suas subscrições, incluindo acesso ao nosso software Red Hat, atualizações, upgrades, suporte técnico, correções de segurança e ao Programa de Garantia de Código Aberto Red Hat. Por favor certifique-se de ler os termos abaixo para entender os termos da sua oferta de renovação porque as configuraçoes do nosso produto mudaram. Se voce tiver alguma duvida, eu sou seu Representante de Vendas Red Hat e voce pode me enviar um email para: <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><a href=mailto:<x:out select="$d//ssirep/email"/>><x:out select="$d//ssirep/email"/></a></x:when><x:otherwise><a href=mailto:<x:out select="$d//client/rep_email"/>><x:out select="$d//client/rep_email"/></a></x:otherwise></x:choose><BR>&nbsp;<BR>
</font></td></tr>
...
<tr><td><font face="Arial" size="2">
<BR>
De acordo com nossas informações, sua subscrição Red Hat vai EXPIRAR EM <c:out value="${stage}"/> DIAS. Por favor certifique-se em renova-las antes da data de expiração para que você continue a recebendo os benefícios de suas subscrições, incluindo acesso ao nosso software Red Hat, atualizações, upgrades, suporte técnico, correções de segurança e ao Programa de Garantia de Código Aberto Red Hat. Por favor certifique-se de ler os termos abaixo para entender os termos da sua oferta de renovação porque as as configuraçoes do nosso produto mudaram. Se voce tiver alguma duvida, eu sou seu Representante de Vendas Red Hat e voce pode me enviar um email para: <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><a href=mailto:<x:out select="$d//ssirep/email"/>><x:out select="$d//ssirep/email"/></a></x:when><x:otherwise><a href=mailto:<x:out select="$d//client/rep_email"/>><x:out select="$d//client/rep_email"/></a></x:otherwise></x:choose><BR>&nbsp;<BR>
</font></td></tr>

</c:otherwise>
</c:choose>

<tr><td><font face="Arial" size="2">Também gostaríamos de aproveitar esta oportunidade para notificar que, baseado em comentários e reações de nossos clientes, nós da Red Hat aperfeiçoamos vários pontos relacionados à sua subscrição para que a mesma se torne mais flexível, acessível e com valor. Uma vez que sua licença esteja renovada, estaremos oferecendo uma versão mais atualizada da solução que você possui atualmente. Essa transição inclui um produto novo com configurações mais flexiveis, números do produto, preço, níveis de suporte e a opção de produtos seccionados. Como resultado das novas configurações, o preço da renovação poderá ser diferente do seu preço original.<br><br></font></td></tr>

<tr><td><font face="Arial" size="2">
Abaixo você poderá encontrar o mapeamento de sua subscrição atual referente ao novo produto que você receberá com sua renovação:</font></td></tr>
...
<table cellspacing="0" cellpadding="0" width="660" border="0">
<tr><td><font face="Arial" size="2">
Você pode encontrar mais informação sobre a estrutura do nosso novo produto visitando, <a href="http://www.redhat.com/rhel/renew/">http://www.redhat.com/rhel/renew/</a>
...
<tr><td><font face="Arial" size="2">
Você pode aprender mais detalhes sobre seus benefícios de renovação de sua subscrição Red Hat <a href ="http://www.redhat.com/subscriptions/" >AQUI</a>.
</font><br><br></td></tr>

<tr><td><font face="Arial" size="2">
Se achar que outra pessoa deveria receber as notificações das suas subscrições Red Hat no futuro, por favor envie os detalhes para <a href=mailto:customerservice-latam@redhat.com>customerservice-latam@redhat.com</a>. Por favor certifique-se de incluir seu Red Hat Login ID e seu número de contrato.   

 <BR>
</font><br></td></tr>

<tr><td><font face="Arial" size="2">
Em caso de dúvidas ou maiores informações, por favor, entre em contato através do telefone: <x:choose><x:when select="$d//subbatch [ . = 'ServiceSource' ]"><x:out select="$d//ssirep/phone"/></x:when><x:otherwise><x:out select="$d//client/rep_tel"/></x:otherwise></x:choose>.
<BR><BR></font></td></tr>
